...
const release = 'https://github.com/Celobra/uo-asset-downloads/releases/download/uo-asset-studio-v0.30.0/';
...
test('The installer is the primary download; application source is separate', async () => {
  for (const page of pages) {
    ...
    assert.equal(primary[0], release + 'UOAssetStudio-Setup-0.30.0.exe');
    assert.ok(primary.every(url => !url.includes('Source-')));
  }
  const downloads = await readFile(localPath('/studio/downloads/'), 'utf8');
  assert.ok(downloads.includes(release + 'UOAssetStudio-Source-0.30.0.zip'));
  assert.ok(downloads.includes(release + 'SHA256SUMS.txt'));
  assert.ok(downloads.includes('/studio/guide/UOAssetStudio-User-Guide-0.30.0.pdf'));
});
...
test('Public guide is complete and uses scripts/styles allowed by CSP', async () => {
  ...
  const html = await readFile(guide + 'UOAssetStudio-User-Guide-0.30.0.html', 'utf8');
  const pdf = await readFile(guide + 'UOAssetStudio-User-Guide-0.30.0.pdf');
  assert.equal(pdf.subarray(0, 5).toString(), '%PDF-');
  assert.equal(createHash('sha256').update(pdf).digest('hex'), 'e450d942a41f223e78ef875640263bddd46098e295aa7dc02e07284041fc7e58', 'PDF must match the published guide without newline conversion');
